...
pairwise_funcs = [a_beats_b, b_beats_a, std_dev_a_beats_b, std_dev_b_beats_a, neither_wins, matchup_count]
...
def offense_evs(a):
  ...
  for spread, spread_weight in md.pokemon[a]["Spreads"].items():
    ...
    offense += (spread_list[1] + spread_list[3]) * spread_weight / md.count_pokemon(a)
...
def defense_evs(a):
  ...
  for spread, spread_weight in md.pokemon[a]["Spreads"].items():
    spread_list = [float(x) for x in spread.split(":")[1].split("/")]
    defense += (spread_list[0] + spread_list[2] + spread_list[4]) * spread_weight / md.count_pokemon(a)
  return defense
